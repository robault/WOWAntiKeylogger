...
namespace Avalon.Windows.Media.Animation
{
        ...
        where T : struct
    {
        ...
        public static readonly DependencyProperty ByProperty = DependencyProperty.Register("By", typeof(T?), typeof(LinearAnimationBase<T>), new PropertyMetadata(new T?(), new PropertyChangedCallback(OnAnimationFunctionChanged)), new ValidateValueCallback(ValidateFromToOrByValue));
        ...
        public static readonly DependencyProperty FromProperty = DependencyProperty.Register("From", typeof(T?), typeof(LinearAnimationBase<T>), new PropertyMetadata(new T?(), new PropertyChangedCallback(OnAnimationFunctionChanged)), new ValidateValueCallback(ValidateFromToOrByValue));
        ...
        public static readonly DependencyProperty ToProperty = DependencyProperty.Register("To", typeof(T?), typeof(LinearAnimationBase<T>), new PropertyMetadata(new T?(), new PropertyChangedCallback(OnAnimationFunctionChanged)), new ValidateValueCallback(ValidateFromToOrByValue));
        ...
        /// <summary>Initializes a new instance of the <see cref="LinearAnimationBase{T}" /> class that animates to the specified value over the specified duration. The starting value for the animation is the base value of the property being animated or the output from another animation.</summary>
        ...
        /// <summary>Initializes a new instance of the <see cref="LinearAnimationBase{T}" /> class that animates to the specified value over the specified duration and has the specified fill behavior. The starting value for the animation is the base value of the property being animated or the output from another animation.</summary>
        ...
        /// <summary>Initializes a new instance of the <see cref="LinearAnimationBase{T}" /> class that animates from the specified starting value to the specified destination value over the specified duration.</summary>
        /// <param name="toValue">The destination value of the animation.</param>
        /// <param name="duration">The length of time the animation takes to play from start to finish, once. See the Duration property for more information.</param>
        ...
        /// <summary>Initializes a new instance of the <see cref="LinearAnimationBase{T}" /> class that animates from the specified starting value to the specified destination value over the specified duration and has the specified fill behavior.</summary>
        /// <param name="fillBehavior">Specifies how the animation behaves when it is not active.</param>
        /// <param name="toValue">The destination value of the animation. </param>
        /// <param name="duration">The length of time the animation takes to play from start to finish, once. See the Duration property for more information.</param>
        /// <param name="fromValue">The starting value of the animation.</param>
        ...
        /// <summary>Creates a modifiable clone of this <see cref="LinearAnimationBase{T}" />, making deep copies of this object's values. When copying dependency properties, this method copies resource references and data bindings (but they might no longer resolve) but not animations or their current values.</summary>
        ...
        protected override T GetCurrentValueCore(T defaultOriginValue, T defaultDestinationValue, AnimationClock animationClock)
        {
            ...
            if (IsCumulative)
            {
                ...
                iteration = (iteration != null) ? iteration.GetValueOrDefault() - 1 : new int?();
                double iterationDouble = (double)iteration.Value;
                if (iterationDouble > 0)
                {
                    cumulative = Calculator.Scale(Calculator.Subtract(destination, origin), iterationDouble);
                }
            }
            return Calculator.Add(additive, Calculator.Add(cumulative, Calculator.Interpolate(origin, destination, progress)));
        }
        ...
        private void ValidateAnimationFunction()
        {
            ...
            if (From != null)
            {
                if (To != null)
                {
                    ...
                    _keyValues = new T[] { From.Value, To.Value };
                }
                else if (By != null)
                {
                    _animationType = AnimationType.FromBy;
                    _keyValues = new T[] { From.Value, By.Value };
                }
                else
                {
                    _animationType = AnimationType.From;
                    _keyValues = new T[] { From.Value };
                }
            }
            else if (To != null)
            {
                _animationType = AnimationType.To;
                _keyValues = new T[] { To.Value };
            }
            else if (By != null)
            {
                _animationType = AnimationType.By;
                _keyValues = new T[] { By.Value };
            }
